...
use st_system::{config::TtsSystemConfig, rvc_backends::seedvc::api::SeedVcApiConfig};
use std::{fmt::Debug, io::Write, path::PathBuf, sync::Arc, time::Duration};

pub type SharedConfig = Arc<SmallTalkConfig>;

pub static CONFIG_FILE: &str = "st_config.toml";
...
pub fn initialise_config<T: Serialize + DeserializeOwned + Default>() -> eyre::Result<T> {
    let c_path = get_full_config_path();

    if !c_path.exists() {
        save_config(&T::default())?;
    }
...
pub fn save_config<T: Serialize>(app_settings: &T) -> eyre::Result<()> {
...
impl<T> SubsystemConfig<T> {
    ...
    pub fn if_enabled(&self) -> Option<&T> {
        if self.enabled { Some(&self.inner) } else { None }
    }
}
