use crate::{
    ...
    tts_backends::{
        ...
        indextts::{
            api::{IndexTtsApiConfig, IndexTtsRequest},
            IndexTts,
        },
    },
};
...
const INDEX_CONTAINER_NAME: &str = "small-talk-index-tts";
...
impl DroppableState for LocalIndexState {
    ...
    async fn initialise_state(context: &Self::Context) -> eyre::Result<Self> {
        let docker_config = DockerTtsCreateConfig {
            container_name: INDEX_CONTAINER_NAME.to_string(),
        ...
        let api = IndexTts::initialise_state(&IndexTtsApiConfig {
